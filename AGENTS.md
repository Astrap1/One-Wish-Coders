...
- **Version 3 additions (2026-09-27), Person 4.** A front near-field LiDAR on the bow bay (`/points_front`, 180° ahead, −25° to +5°, 0.2–15 m) covers the mast LiDAR's blind zone in front of the bow; it is bridged but **not yet merged into `/scan`**. New options, all **off by default** and not yet switched on for any vehicle: `vehicle_mobility_node` command ramps (`max_linear_accel_mps2`, `max_yaw_accel_rps2`; slowing down is never delayed), IMU attitude (`use_imu_attitude`), a downhill speed cap from the braking left after gravity (`hover_brake_decel_mps2`), and track deployment to brake on steep firm descents (`track_descent_slope_deg`); and in `lidar_scan_node` a map-free slope ground filter levelled by the IMU (`slope_ground_filter_deg`, `level_with_imu`). The corridor-profile ground filter (Person 1) stays the active one; the slope filter is kept for worlds whose profile is not built in. TRACK mode now leaves only after the climb stays below `track_exit_slope_deg` for `track_exit_dwell_s` (V2 10° / V3 4°), so one bump on a bank no longer retracts the tracks mid-climb.
