...
- Added 34 ROS-independent planner, follower, motion-limit and LiDAR tests and eight ROS topic integration tests (42 Autonomy tests total).
...
12. **Version 3 LiDAR self-filter (Person 4).** In the corrected live corridor run, the closest false obstacle returns included LiDAR-frame points `(-1.56, -0.11, -0.42)` and `(-0.99, 1.27, -0.43)`. The first lies exactly on the configured `self_box_m` boundary and the second remains inside the vehicle's roughly 1.75 m corner envelope. They were projected onto traversable cells near the vehicle and inflated into a temporary barrier. Make the V3 self-filter inclusive and cover the complete moving vehicle envelope (prefer a tested footprint/radius mask over hand-tuned edge values), then repeat the corridor mission. Autonomy must not ignore genuine external returns to compensate.
