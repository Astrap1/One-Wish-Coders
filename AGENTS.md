...
19. **NOTICE, done by Person 4 in your models (2026-09-27): mangrove meshes moved onto their collision proxies.** The Mangrovetree GLB's trunk is not at its origin but at (2.85, −1.86) m unscaled, so after the switch to cylinder collision proxies every visible tree sat 8–18 m away from its proxy and its `/terrain_costmap` footprint (for example `mangrove_18`'s tree was drawn at (74.4, 0.2), on the main route). Gazebo's GPU LiDAR sees visual meshes, so these "phantom trees" produced LiDAR obstacles on cells the map calls free. Each `mangrove_*/model.sdf` visual link now carries the scaled offset, so the tree is drawn where it collides. Please review in the GUI. The prop roots still reach about 0.85 × scale m from the trunk, wider than the proxies (0.75–1.3 m), so you may want larger proxies and footprints.
...
Mode selection is internal to Person 4's controller (`vehicle_mobility_node`, `gear: tracks`, `mode_policy: terrain_auto`). Version 2 is **hover-first**: it hovers across firm shore, mud and water, with its tracks retracted once the cushion is ready. It switches to TRACK only for a sustained (> 1.5 s) forward climb at or above 14° on firm land. This 1° guard band prepares it for the physical 15° bank. There the cushion carries 60% of the load while the tracks provide traction, and it returns to HOVER when the climb ends (below 10° for 0.5 s, or at once over water or off firm ground). Side tilt, stopping, pivoting, descending, mud and water do not deploy the tracks. The cost bands below still define route risk and Safety's energy estimates. See `docs/VEHICLE_SIMULATION.md` (*Mode selection*).
...
- V3's current LiDAR-only profile, authorized by the team, makes `/terrain_costmap` operator-reference only. Neither Autonomy nor Safety subscribes to or calculates route geometry, speed limits, return validity, energy or state changes from it. `global_planner` uses a bounded 1 m free-space grid as A* geometry and marks confirmed, inflated LiDAR returns as no-go; it uses the full 30 m LiDAR horizon at every speed. Safety estimates a validated LiDAR return using its conservative hover-energy model without terrain cells. The V3 follower has terrain-zone speed limits disabled, and is conservatively capped at 1.2 m/s forward and 0.30 rad/s yaw while mobility/PID behaviour is tuned. Sensor-range, curvature, yaw-damping, battery/fuel, telemetry and vehicle-fault safeguards remain active. Versions 1 and 2 are unchanged.
- Person 1 route-stability update (2026-09-27): V3 associates a new LiDAR surface cell within 1.5 m of a confirmed obstacle to that obstacle's existing grid centre. This stops normal changes in viewing angle around a tree or rock from moving its 2 m inflated overlay and selecting an equally valid but radically different A* detour. Replan checks also trim the already-travelled route prefix at the vehicle's current odometry cell, so a late scan update behind the vehicle cannot reset its forward route. New obstacles that intersect the remaining outbound or return corridor still replan immediately; detection range and clearance are unchanged.
- During this LiDAR-only V3 regression, Person 1 corrected a launch-blocking sensor-call defect in `lidar_scan_node`: the terrain-relative height array and scalar clearance had been passed positionally after the optional footprint-radius alias. The call now uses explicit names, preserving V3's 0.30--1.30 m terrain-relative obstacle slice. This is a sensor-conversion compatibility repair only; it does not change Environment terrain or vehicle physics.
- Verification before this Safety override: all 114 workspace tests passed. In a headless V3 launch, `lidar_only_navigation=True`, `/scan` stayed live at about 3 Hz, and the planner published an 89--93-cell route from confirmed LiDAR obstacles. Gazebo later stopped advancing ROS topics while using about 66% CPU, so the run was stopped before a movement-quality measurement; re-run the headless test after the host/Gazebo slowdown is investigated before raising V3's conservative cap. After the override, the Autonomy and Safety suites pass (70 tests).
...
- **Version 3 additions (2026-09-27), Person 4.** A front near-field LiDAR on the bow bay (`/points_front`, 180° ahead, −25° to +5°, 0.2–15 m) is **merged into `/scan`** for Version 3 (`front_cloud_topic`), covering the mast LiDAR's blind zone in front of the bow. **Person 1: nothing to change** (same topic, frame and format), but `/scan` now shows low obstacles just ahead of the bow. Also **on for Version 3** (`config/vehicle_mobility_v3.yaml`): command ramps (`max_linear_accel_mps2` 0.5, `max_yaw_accel_rps2` 0.4; slowing down is never delayed); slopes from the IMU (`use_imu_attitude`); a downhill speed cap from the braking left after gravity (`hover_brake_decel_mps2` 1.42); and track deployment to brake on firm descents of 7° or more (`track_descent_slope_deg`). Versions 1 and 2 keep all of these off. `lidar_scan_node` also has a map-free slope ground filter levelled by the IMU (`slope_ground_filter_deg`, `level_with_imu`), **off**: the corridor-profile ground filter (Person 1) stays the active one. TRACK mode now leaves only after the climb stays below `track_exit_slope_deg` for `track_exit_dwell_s` (V2 10° / V3 4°), so one bump on a bank no longer retracts the tracks mid-climb.
