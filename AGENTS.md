...
1. **Corridor incline and water.** The corridor collision surface (`demo_terrain`) is tilted 4.6° (pitch 0.08 rad), so the ground is already below z = 0 a metre past HOME. Two vehicle faults this exposed are fixed: Gazebo's world-wide buoyancy plane at z = 0 floated Version 2 off its tracks (its buoyancy entries are removed from the corridor), and the propulsion fans braked against the tracks in TRACK mode (they now idle while a load share is active). In a Gazebo-only run of the corridor world, Version 2 now drives 0.40 m/s downhill and pivots at 0.50 rad/s on its tracks. Still open for Person 3: `TerrainZones` puts the channel water at z = 0 from x = 4 m, where the tilted ground is at about −0.32 m, so a hovering vehicle meets a 0.32 m water "step" at the channel edge (it stalled at x ≈ 3.6 m). The visual water sheet starts at z = −1.80 m and rises 2.4 m. Proposed fix: make the physics water follow the visual sheet (`<level>-1.80</level><rise>2.40</rise><rise_start>0</rise_start><rise_duration>30</rise_duration>`, or the new tide timing), or flatten the shore. Version 1's buoyancy entries in the corridor have the same floating problem.
