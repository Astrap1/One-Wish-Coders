...
<style>
:root{color-scheme:dark;--ink:#eef3ff;--muted:#839bb9;--cyan:#63d9ff;--panel:#101b33;--line:#24496a}*{box-sizing:border-box}body{margin:0;background:#070d1a;color:var(--ink);font-family:Arial,sans-serif;background-image:radial-gradient(circle at 50% -20%,#173a56 0%,#070d1a 56%)}main{max-width:1280px;margin:auto;padding:24px}.head{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:16px}.eyebrow,.label{color:var(--cyan);font-size:12px;font-weight:700;letter-spacing:1.6px;text-transform:uppercase}.live,.sub,.legend{color:var(--muted);font-size:13px}.dot{color:#35e28b}h1{margin:6px 0 0;font-size:28px;letter-spacing:1px;text-transform:uppercase}.grid{display:grid;grid-template-columns:repeat(6,1fr);gap:14px}.card{position:relative;overflow:hidden;padding:16px;min-height:108px;background:linear-gradient(145deg,#111d36,#0c152a);border:1px solid var(--line);border-radius:12px;box-shadow:0 12px 32px rgba(0,0,0,.22)}.card:before{content:"";position:absolute;top:0;left:16px;right:16px;height:2px;background:linear-gradient(90deg,transparent,#2c9ec1,transparent);opacity:.45}.wide{grid-column:span 6}.span3{grid-column:span 3}.span2{grid-column:span 2}.span1{grid-column:span 1}.value{margin-top:12px;font-size:28px;font-weight:700;letter-spacing:-.5px}.value.compact{font-size:24px}.status{display:inline-block;margin-top:14px;padding:7px 12px;border:1px solid #38d6ff;border-radius:999px;background:#142d43;color:#78e5ff;font-weight:700;letter-spacing:1px}.metric-pair{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:12px}.metric-pair .value{margin-top:3px}.metric-label{color:#8da7c2;font-size:11px;font-weight:700;letter-spacing:1px;text-transform:uppercase}.metric-note{margin-top:8px;color:#839bb9;font-size:12px}.map{display:block;width:100%;height:390px;margin-top:10px;background:#101a30;border:1px solid #214967;border-radius:8px}.tide{display:block;width:100%;height:180px;margin-top:10px;background:#101a30;border:1px solid #214967;border-radius:8px}.camera{display:block;width:min(100%,640px);aspect-ratio:16/9;object-fit:cover;margin:10px auto 0;background:#101a30;border:1px solid #214967;border-radius:8px}.legend{display:flex;flex-wrap:wrap;gap:14px;margin-top:8px}.blue{color:#42a5ff}.orange{color:#ffc857}.red{color:#f52222}.green{color:#35e28b}.map-legend{font-size:12px}.event-list{height:198px;overflow:auto;display:flex;flex-direction:column;gap:8px;margin-top:10px}.event-item{padding:8px 10px;border-left:2px solid #42d9ff;background:#101a30;color:#d7e6f7;font:12px ui-monospace,monospace}.event-time{color:#63d9ff;margin-right:8px}.remote-toggle,.remote-abort{display:block;margin:12px auto;padding:9px 16px;border:1px solid #48d8ff;border-radius:6px;background:#10243a;color:#8ceaff;font-weight:700;cursor:pointer}.remote-toggle.active{background:#1e5c71;color:#fff}.remote-abort{border-color:#bd5360;background:#351e2a;color:#ff9ba4}.remote-pad{width:190px;height:190px;margin:14px auto;position:relative;border:1px solid #2d5370;border-radius:50%;background:radial-gradient(circle,#152941 0 31%,transparent 32%),linear-gradient(45deg,transparent 49%,#2d5370 50%,transparent 51%),linear-gradient(-45deg,transparent 49%,#2d5370 50%,transparent 51%)}.remote-pad.disabled{opacity:.35;pointer-events:none}.remote-btn{position:absolute;width:42px;height:42px;border:1px solid #48d8ff;border-radius:50%;background:#10243a;color:#8ceaff;font-size:22px;cursor:pointer}.remote-up{top:8px;left:74px}.remote-down{bottom:8px;left:74px}.remote-left{left:8px;top:74px}.remote-right{right:8px;top:74px}.remote-stop{top:74px;left:74px;color:#ff8790;border-color:#bd5360;background:#351e2a;font-size:15px}.remote-status,.remote-note{text-align:center;color:#8da7c2;font-size:12px}.remote-note{color:#62809e;margin-top:6px}@media(max-width:1000px){.grid{grid-template-columns:repeat(2,1fr)}.wide{grid-column:span 2}.span3,.span2{grid-column:span 1}.span1{grid-column:span 1}}@media(max-width:640px){main{padding:16px}.head{gap:12px;flex-direction:column}.grid{grid-template-columns:1fr}.card,.wide,.span1,.span2,.span3{grid-column:1 / -1}.map{height:280px}}
...
<article class="card span1"><div class="label">Ground speed</div><div class="value compact" id="speed">—</div><div class="sub">Approved motion</div></article>
<article class="card span1"><div class="label">Vehicle position</div><div class="value compact" id="coordinates">—</div><div class="sub">Map frame metres</div></article>
<article class="card span1"><div class="label">Battery reserve</div><div class="value compact" id="battery">—</div><div class="sub">Electrical charge</div></article>
<article class="card span1"><div class="label">Fuel reserve</div><div class="value compact" id="fuel">—</div><div class="sub" id="fuel-note">Vehicle fuel state</div></article>
<article class="card span2"><div class="label">Return reserve</div><div class="value" id="reserve">—</div><div class="metric-note" id="reserve-note">Battery margin after the predicted trip HOME.</div></article>
<article class="card span2"><div class="label">Predicted return demand</div><div class="metric-pair"><div><div class="metric-label">Energy to HOME</div><div class="value compact" id="return-energy">—</div></div><div><div class="metric-label">Return ETA</div><div class="value compact" id="return-eta">—</div></div></div><div class="metric-note">Terrain and mobility-adjusted estimate.</div></article>
<article class="card span2"><div class="label">Tide safety window</div><div class="metric-pair"><div><div class="metric-label">Water level</div><div class="value compact" id="tide-level">—</div></div><div><div class="metric-label">Time remaining</div><div class="value compact" id="tide-window">—</div></div></div><div class="metric-note" id="tide-state">—</div></article>
...
<article class="card span3"><div class="label">Tide level over time</div><canvas id="tide-chart" class="tide"></canvas><div class="metric-note">Live water level from the tidal-corridor simulation.</div></article>
...
const E=id=>document.getElementById(id),N=v=>Number.isFinite(Number(v))?Number(v):0,F=v=>typeof v==='number'&&Number.isFinite(v)?v:null,T=[];let remoteMode=false;
...
function percent(value){const n=F(value);return n===null?'—':n.toFixed(1)+'%'}function duration(value){const n=F(value);if(n===null||n<0)return 'OPEN';const seconds=Math.round(n),minutes=Math.floor(seconds/60);return minutes?minutes+'m '+String(seconds%60).padStart(2,'0')+'s':seconds+' s'}
async function refresh(){try{const d=await(await fetch('/api/state',{cache:'no-store'})).json(),t=d.telemetry||{},level=N(t.water_level_m),fuel=F(t.fuel_percent),windowS=F(t.seconds_until_corridor_unsafe);setRemote(d.remote_enabled);E('battery').textContent=percent(d.battery_percent);E('fuel').textContent=fuel===null||fuel<0?'N/A':percent(fuel);E('fuel-note').textContent=fuel===null||fuel<0?'No fuel tank modelled':'Series-hybrid fuel remaining';E('reserve').textContent=percent(d.return_margin_percent);E('reserve-note').textContent='Margin after '+percent(t.estimated_return_energy_percent)+' predicted energy to HOME.';E('return-energy').textContent=percent(t.estimated_return_energy_percent);E('return-eta').textContent=duration(t.estimated_return_time_s);E('speed').textContent=N(t.speed_mps).toFixed(2)+' m/s';E('coordinates').textContent=N(t.vehicle_x).toFixed(1)+', '+N(t.vehicle_y).toFixed(1);E('safety').textContent=d.return_required?'RETURN':(d.mission_state||'HOLD');E('reason').textContent=d.reason||'—';E('tide-level').textContent=level.toFixed(2)+' m';E('tide-window').textContent=windowS===null||windowS<0?'OPEN':duration(windowS);E('tide-state').textContent=(t.tide_state||'UNKNOWN')+' · risk '+(N(t.tide_risk)*100).toFixed(0)+'% · '+(t.corridor_traversable?'corridor traversable':'corridor constrained');E('connection').textContent='Live telemetry';E('camera-status').textContent='Live camera stream';T.push(level);if(T.length>72)T.shift();drawMap(t);drawTide();events(t.mission_events)}catch(_){E('connection').textContent='Telemetry unavailable'}}setInterval(refresh,750);setInterval(()=>{E('camera').src='/api/camera.jpg?ts='+Date.now()},1000);refresh();
</script></body></html>"""
