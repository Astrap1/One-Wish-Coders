...
<article class="card wide"><div class="label">Gazebo top-down map &amp; route</div><canvas id="map" class="map"></canvas><div class="legend"><span class="blue">● Vehicle</span><span class="blue">━ Planned path</span><span class="orange">━ Return path</span><span>terrain background</span></div></article>
</section></main><script>
...
function drawCostMap(t){const c=E('cost-map'),r=c.getBoundingClientRect(),d=devicePixelRatio||1,m=t.cost_map||{};c.width=r.width*d;c.height=r.height*d;const x=c.getContext('2d');x.setTransform(d,0,0,d,0,0);x.clearRect(0,0,r.width,r.height);if(!m.width||!m.height||!m.data?.length){x.fillStyle='#839bb9';x.font='12px Arial';x.fillText('Waiting for /terrain_costmap',16,24);return}const cellW=r.width/m.width,cellH=r.height/m.height;for(let row=0;row<m.height;row++)for(let col=0;col<m.width;col++){const v=m.data[row*m.width+col];x.fillStyle=v<0?'#26334a':v>=90?'#c74652':`hsl(${190-(Math.min(v,89)/89)*150},75%,${35+(1-Math.min(v,89)/89)*25}%)`;x.fillRect(col*cellW,r.height-(row+1)*cellH,Math.ceil(cellW)+1,Math.ceil(cellH)+1)}}
