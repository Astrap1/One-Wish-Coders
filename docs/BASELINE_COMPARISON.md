...
| Rising tide | Reduces traversable shore/mud area | May improve flotation route but can create new hazards | Updates terrain state/cost map and triggers route reassessment; water remains traversable in hover mode. A separate blocked route, energy limit or vehicle fault triggers hold/return. |
...
3. Rising-tide terrain updates that cause a visible route reassessment and replan while the hovercraft continues across traversable water.
