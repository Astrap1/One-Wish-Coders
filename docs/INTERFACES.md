...
| `/points_front` | Simulation (Version 3 only) | Operator (view); not yet merged into `/scan` | `sensor_msgs/PointCloud2` | Front near-field LiDAR on the bow bay, frame `lidar_front_link`, 180° ahead, 8 beams from −25° to +5°, 10 Hz, 0.2–15 m. Covers the mast LiDAR's blind zone in front of the bow. |
