...
| --- | --- | --- | --- | --- |
| `/scan` | Simulation (`lidar_scan_node`) | Autonomy | `sensor_msgs/LaserScan` | Best-effort sensor QoS. Near-field obstacle sensing used for the planner's temporary obstacle overlay. 2D scan in `lidar_link`, 720 bins, derived from `/points` (closest return per bearing in the configured height band; the vehicle's own body is filtered with inclusive footprint bounds, plus a tested 1.85 m radial envelope on V3). V3 uses `/odom` and the known corridor profile to retain only 0.30–1.30 m terrain protrusions: it sees rocks and trunk collision geometry, but rejects the embankment and overhead visual canopy. On V3 it also merges the front near-field LiDAR (`/points_front`, shifted into `lidar_link`), so low obstacles just ahead of the bow appear too; same frame and format. 10 Hz simulated. |
...
| `/points_front` | Simulation (Version 3 only) | Simulation (`lidar_scan_node`, merged into `/scan`), Operator (view) | `sensor_msgs/PointCloud2` | Front near-field LiDAR on the bow bay, frame `lidar_front_link`, 180° ahead, 8 beams from −25° to +5°, 10 Hz, 0.2–15 m. Covers the mast LiDAR's blind zone in front of the bow. |
...
Autonomy assumes the LiDAR origin matches the odometry position and its zero angle points along the vehicle's forward axis; this horizontal transform is exact for Versions 2 and 3. The shared launch uses 1.7 m inflation for V2 and 2.0 m for V3. V3 grows useful planning range from 8 m to the 30 m sensor limit using stopping distance plus clearance, and its follower limits autonomous speed to what can stop within the remaining 28 m.
