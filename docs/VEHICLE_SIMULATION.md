...
| `tidal_vehicle_simulation/scripts/vehicle_mobility_node.py` | `/cmd_vel` → fans or wheels, WHEEL/TRANSITION/HOVER mode switching, `/vehicle_health`, placeholder `/terrain_state` |
...
Mobility modes follow `AGENTS.md`: `WHEEL`, `TRANSITION` and `HOVER` (Version 2 replaces `WHEEL` with `TRACK`). The current mode is published read-only on `/vehicle/mode`. During `TRANSITION` the vehicle ignores `/cmd_vel` and stays stopped.

- **Hover-ready gate:** a transition into `HOVER` completes only when the air-cushion plugin reports `hover_state == HOVER`. If that doesn't happen within `transition_timeout_s` (10 s), the vehicle stays stopped and `/vehicle_health.fault` reports `hover_not_ready`, which Safety treats as `HOLD`.
- **`hover_only` (default):** at start-up the lift fan spins up, the legs fold, and from then on `/cmd_vel` drives the fans.
- **`terrain_auto`:** starts in WHEEL mode on the wheels.
  ...
  - The HOVER→WHEEL transition while moving is **not yet validated** (planned test). Use `hover_only` for demos.
